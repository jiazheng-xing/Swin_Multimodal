...
def main():
    ...
    if config.fewshot:
       #冻结参数
        ...
        for fusion_param in fusion_model.parameters():
            fusion_param.requires_grad = False

        #添加fc
        class Few_shot_Net(nn.Module):
            def __init__(self, model):
                super(Few_shot_Net, self).__init__()
                self.model = model
                self.dimension = 512
                # self.fc = nn.Sequential(
                #     nn.Linear(self.dimension, int(self.dimension / 4)).half(),
                #     nn.Linear(int(self.dimension / 4), self.dimension).half()
                # )
                self.fc =  nn.Linear(self.dimension, self.dimension ).half()
                # self.fc1 = nn.Linear(self.dimension, int(self.dimension / 4)).half()
                # self.fc2 = nn.Linear(int(self.dimension / 4), self.dimension).half()
                #self.relu = nn.ReLU()
                self.alpha = 0.6
            ...
            def forward(self, x):
                ...
                x = self.fc(x)
                #x = self.relu(x)
                x = self.alpha * x + (1 - self.alpha) * orign
                return  x
        fusion_model = Few_shot_Net(fusion_model.module)
        print('-----------------------------------few_shot-------------------------------------------------')
        fusion_model = torch.nn.DataParallel(fusion_model).cuda()
    ...
    for epoch in range(start_epoch, config.solver.epochs):
        ...
        for kkk,(images,list_id) in enumerate(tqdm(train_loader)):
            ...
            text_embedding = model_text(texts) #(96,77,512)
            # image_embedding = fusion_model(image_embedding)
            # co_embeds = (image_embedding, text_embedding, texts)

            image_embedding, text_embedding = fusion_model(image_embedding, text_embedding, texts) #(96,512)
            ...
            total_loss = (loss_imgs + loss_texts)
