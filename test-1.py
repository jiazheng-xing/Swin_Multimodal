...
def validate(epoch, val_loader, classes, device, model, fusion_model, config, num_text_aug):
    ...
    with torch.no_grad():
        ...
        text_features = model.encode_text(text_inputs) #(400,N,512)
        # text_features = text_features.unsqueeze(1) # 1_tk
        # import pdb
        # pdb.set_trace()
        for iii, (image, class_id) in enumerate(tqdm(val_loader)):
            ...
            # import pdb
            # pdb.set_trace()
            # text_blank = torch.zeros(b,image_features.shape[2], device=text_features.device)
            # image_features,text_blank = fusion_model(image_features, text_blank)
            co_similarity = []
            for jj  in range(text_features.shape[0]):     
                ...
                # sam_text_features = sam_text_features.squeeze(1)  # 1_tk
                sam_image_features, sam_text_features = fusion_model(image_features_in , sam_text_features, sam_text_inputs)
                # co_text_features.append(sam_text_features)
                sam_image_features /= sam_image_features.norm(dim=-1 , keepdim = True)
                sam_text_features /= sam_text_features.norm(dim=-1, keepdim = True)
                sam_similarity = (100 * sam_image_features @ sam_text_features.T)
                sam_similarity = torch.diag(sam_similarity)
                co_similarity.append(sam_similarity)
            similarity = (torch.stack(co_similarity)).T

            # print("----------------------------------------------------it's the testing-time--------------------------------------------------------------")
            # text_features =  torch.stack(co_text_features)
            # import pdb
            # pdb.set_trace()
