...
# srun -N 1 --cpus-per-task=16 -p huzhou-1 --gres=gpu:4 --reservation wangmm_11 --qos high --job-name test -t 7-00:00:00 python -u train_OR.py --config ${config} --log_time $now 2>&1|tee exp/${type}/${arch}/${dataset}/${now}/$now.log
