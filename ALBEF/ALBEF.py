...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha, idx):
        ...
        text_embeds_all = torch.cat([text_embeds, text_embeds_neg],dim=0)   #注意这个地方是颠倒的
        text_atts_all = torch.cat([text.attention_mask, text_atts_neg],dim=0)     

        image_embeds_all = torch.cat([image_embeds_neg,image_embeds],dim=0) #注意这个地方是颠倒的
