...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD x
        x = self.ln_final(x).type(self.dtype) #48,77,512
        # x = x[:,0:30,:]
        ...
        x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @  self.text_projection #(48,512)*(512,512)  #可以考虑以下要不要做这步
        return x
...
def build_model(state_dict: dict, tsm=False,T=8,dropout=0., joint=False,emb_dropout=0.,pretrain=True):
    ...
    vocab_size = state_dict["token_embedding.weight"].shape[0] #限制死了vocab_size的长度
