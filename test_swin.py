...
def validate(epoch,val_loader, classes, device, model,swin_model, config, num_text_aug):
    model.eval()
    swin_model.eval()
    num = 0
    corr_1 = 0
    ...
    with torch.no_grad():
        ...
        for iii, (image, class_id) in enumerate(tqdm(val_loader)):
            ...
            image_input = image.to(device).permute(0,2,1,3,4)
            image_features = swin_model(image_input)
